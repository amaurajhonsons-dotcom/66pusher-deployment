...
class App {
    ...
    public function __construct() {
        ...
        if(is_logged_in()) {
            ...
            if($user->plan_settings->campaigns_per_month_limit != -1 && $user->pusher_campaigns_current_month > $user->plan_settings->campaigns_per_month_limit && !isset($_COOKIE['plan_campaigns_limit_notice'])) {
            ...
            if($user->plan_settings->campaigns_per_month_limit != -1 && $user->pusher_sent_push_notifications_current_month > $user->plan_settings->campaigns_per_month_limit && !isset($_COOKIE['plan_sent_push_notifications_limit_notice'])) {
                Alerts::add_warning('<strong>' . l('global.notifications.user_sent_push_notifications_limit.title') . '</strong>' . (settings()->payment->is_enabled ? '<br />' . sprintf(l('global.notifications.user_sent_push_notifications_limit.description'), '<a href="' . url('plan') . '" class="font-weight-bold">', '</a>') : null));
                setcookie('plan_sent_push_notifications_limit_notice', 1, time()+60*60*24*1, COOKIE_PATH);
            }
        }
