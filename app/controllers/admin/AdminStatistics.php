...
class AdminStatistics extends Controller {
    ...
    protected function pwas() {
        ...
        while($row = $result->fetch_object()) {
            ...
            $pwas_chart[$row->formatted_date] = [
                'pwas' => $row->total
            ];

            $total['pwas'] += $row->total;
        }

        $pwas_chart = get_chart_data($pwas_chart);

        return [
            'total' => $total,
            'pwas_chart' => $pwas_chart,
        ];
    ...
    protected function personal_notifications() {

        $total = ['personal_notifications' => 0];
        ...
        while($row = $result->fetch_object()) {
            ...
            $personal_notifications_chart[$row->formatted_date] = [
                'personal_notifications' => $row->total
            ];

            $total['personal_notifications'] += $row->total;
        }
        ...
        return [
            'total' => $total,
            'personal_notifications_chart' => $personal_notifications_chart,
        ];
