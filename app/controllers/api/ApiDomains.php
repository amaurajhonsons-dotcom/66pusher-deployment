...
class ApiDomains extends Controller {
    ...
    private function post() {
        ...
        if(in_array($_POST['host'], settings()->websites->blacklisted_domains)) {
            $this->response_error(l('websites.error_message.blacklisted_domain'), 401);
        }

        if(!empty($_POST['custom_index_url']) && in_array(get_domain_from_url($_POST['custom_index_url']), settings()->websites->blacklisted_domains)) {
            $this->response_error(l('websites.error_message.blacklisted_domain'), 401);
        }

        if(!empty($_POST['custom_not_found_url']) && in_array(get_domain_from_url($_POST['custom_not_found_url']), settings()->websites->blacklisted_domains)) {
            $this->response_error(l('websites.error_message.blacklisted_domain'), 401);
        }
